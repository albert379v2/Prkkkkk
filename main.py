...
def procesar_tarjeta(card: str, route=None) -> dict:
    try:
        ...
        return {
            "status": "error",
            "message": "Formato inválido. Usa: cc|mm|yy|cvv",
            "card": card
        }
...
def check_card():
    """
    POST /check
    Body JSON: {"card": "1234567890123456|12|28|123"}
    """
...
def check_bulk():
    """
    POST /check/bulk
    Body JSON: {"cards": ["cc|mm|yy|cvv", "cc|mm|yy|cvv"]}
    """
    data = request.get_json(silent=True) or {}
    ...
    if not cards or not isinstance(cards, list):
        return jsonify({
            "status": "error",
            "message": "Falta el campo 'cards' (array)"
        }), 400
